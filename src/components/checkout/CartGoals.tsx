...
import { GoalBar, GoalDeadline, GoalProgressText } from "@/components/money/GoalUI"
import { formatPts } from "@/components/money/PointsUI"
import { formatGoalAmount, goalHeadline, goalStatuses, scopeLink, type GoalStatus } from "@/data/venadoMoney"
    ...
    <section>
      <div className="mb-3 flex items-center gap-2">
        ...
        <div className="flex flex-col">
          <h2 className="text-base leading-tight">Tus objetivos</h2>
      ...
      <div className={cn("flex gap-3", !single && "no-scrollbar -mx-4 snap-x overflow-x-auto px-4 pb-1")}>
        {statuses.map((status) => (
          <CartGoalCard
            key={status.strategy.id}
            status={status}
            multiplier={tier.multiplier}
            className={single ? "w-full" : "w-[86%] shrink-0 snap-start sm:w-80"}
          />
        ))}
      </div>
    </section>
  ...
  return (
    <div
      className={cn(
        ...
        completesNow ? "bg-success/10 ring-success/40" : "bg-money/15 ring-money/40",
        className
      )}
    >
      <div className="flex items-start justify-between gap-2">
        <span className="line-clamp-2 text-[13px] leading-snug font-semibold">{goalHeadline(strategy)}</span>
      ...
      {completesNow ? (
        <div className="flex items-center gap-2 rounded-xl bg-success px-3 py-2 text-primary-foreground">
          ...
          <span className="shrink-0 text-[13px] font-bold tabular-nums">+{formatPts(points)}</span>
        </div>
      ) : (
        <Link
        ...
        >
          <span className="min-w-0 flex-1 text-[13px] leading-tight font-semibold">
            Te faltan <strong>{formatGoalAmount(missing)}</strong> para ganar {formatPts(points)}
          </span>
