...
export const TIERS: Tier[] = [
  {
    ...
    max: 1000,
    multiplier: 1,
    benefit: "Puntos base de cada objetivo",
  },
  {
    key: "plata",
    label: "Plata",
    ...
    max: 5000,
    multiplier: 1.2,
    benefit: "20 % más de puntos por objetivo",
  },
  {
    key: "oro",
    label: "Oro",
    ...
    max: null,
    multiplier: 1.5,
    benefit: "50 % más de puntos por objetivo",
  },
];
...
export function progressToNext(lifetime: number): {
  tier: Tier;
  next: Tier | null;
  pct: number;
  missing: number;
...
export function strategyMatches(s: EarnStrategy, product: Product): boolean {
  switch (s.scope.kind) {
    ...
    case "subcategory":
      return (
        product.categoryId === s.scope.categoryId &&
        product.subcategoryId === s.scope.subcategoryId
      );
    case "category":
...
export function scopeLabel(s: EarnStrategy): string {
  ...
  switch (scope.kind) {
    case "product":
      return scope.productIds.length > 1
...
export function scopeSampleProduct(s: EarnStrategy): Product | undefined {
  return (
    products.find((p) => strategyMatches(s, p) && hasProductImage(p)) ??
    products.find((p) => strategyMatches(s, p))
  );
}
...
export function goalStatus(
  ...
  const daysLeft = s.endsAt
    ? Math.max(0, Math.floor((s.endsAt.getTime() - Date.now()) / DAY_MS))
    : null;
  return {
    strategy: s,
    before,
...
export function goalStatuses(
  ...
  return EARN_STRATEGIES.map((s) => goalStatus(s, progress, quote)).sort(
    (a, b) => {
...
export function earnBreakdown(
  ...
  return {
    ...
    inProgress,
    base,
    total: Math.floor(base * tier.multiplier),
  };
}

/** Puntos que deja un pedido (los canjes no suman). */
export function pointsForQuote(
  quote: Pick<Quote, "lines">,
  tier: Tier,
  progress: GoalProgressMap,
): number {
...
export function applyQuoteToGoals(
  progress: GoalProgressMap,
  quote: Pick<Quote, "lines">,
): GoalProgressMap {
...
export function pointsCost(product: Product): number {
  return Math.max(
    5,
    Math.round(mockPrice(product) / REDEEM_BS_PER_POINT / 5) * 5,
  );
}
...
export const REDEEM_CATEGORY_IDS = [
  "salsas",
  "bebidas-rtd",
  "bebidas-en-polvo",
  "limpieza-del-hogar",
...
const redeemableProducts: Redeemable[] = REDEEM_CATEGORY_IDS.flatMap(
  (categoryId) =>
    products
...
export const redeemCategories: { id: string; label: string }[] = [
  ...categories
    .filter((c) => REDEEM_CATEGORY_IDS.includes(c.id))
