...
import {
  ...
  TierBadge,
  formatPts,
} from "@/components/money/PointsUI";
...
export function PointsHome() {
  ...
  const affordableFirst = [...redeemables]
    .sort((a, b) => Number(b.points <= balance) - Number(a.points <= balance))
    .slice(0, 8);
  ...
  return (
    <div className="flex flex-col gap-7 px-4 pt-3 pb-8">
        ...
        <div className="relative z-10 flex items-center justify-between gap-4">
          <div className="flex min-w-0 flex-col gap-2">
            ...
            <div className="flex flex-col leading-none">
              <span className="cn-font-heading text-5xl tabular-nums">
          ...
          <ProgressRing
            value={progress.pct}
            size={104}
            className="text-money-foreground"
          >
            <span className="cn-font-heading text-xl leading-none">
              {Math.round(progress.pct * 100)}%
            </span>
            <span className="text-[10px] font-semibold opacity-80">
              {progress.next ? `a ${progress.next.label}` : "máximo"}
            </span>
          </ProgressRing>
        </div>
        {progress.next && (
          ...
          >
            <span>
              Te faltan <strong>{formatPts(progress.missing)}</strong> para
              llegar a {progress.next.label}
            </span>
        ...
        <div className="-mt-3 flex items-center gap-2 rounded-2xl bg-card px-3 py-2 text-[11px] ring-1 ring-foreground/5">
          ...
          <span className="min-w-0 flex-1 truncate text-muted-foreground">
            ...
            <strong className="text-foreground">
              {formatPts(nextExpiry.remaining)}
            </strong>{" "}
            el{" "}
            {nextExpiry.expiresAt
              ...
              .replace(/\./g, "")}{" "}
            · vencen a los {EXPIRY_MONTHS} meses
          </span>
        </div>
      )}
        ...
        {[
          {
            ...
            icon: Gift,
            label: "Canjear",
            hint: `${redeemables.length} productos y premios`,
          },
          {
            to: "/puntos/extracto",
            icon: ReceiptText,
            label: "Extracto",
            hint: `${movements.length} movimientos`,
          },
          {
            to: "/puntos/objetivos",
            icon: Target,
            label: "Suma puntos",
            hint: `${activeGoals.length} en curso`,
          },
          {
            to: "/puntos/niveles",
            icon: Medal,
      ...
      <section>
        <SectionHeader
          title="Canjeá con tus puntos"
          subtitle="Se suman a tu próximo pedido, sin costo en Bs"
          to="/puntos/canjear"
          linkLabel="Ver todo"
        />
      ...
      <section>
        ...
        <div className="flex flex-col gap-2">
          {(activeGoals.length > 0 ? activeGoals : goalList)
            .slice(0, 3)
            .map((status) => (
              <GoalRow
        ...
        <div className="flex flex-col divide-y divide-foreground/5 rounded-3xl bg-card ring-1 ring-foreground/5">
          {movements.slice(0, 3).map((m) => (
            <div key={m.id} className="flex items-center gap-3 px-4 py-3">
              <span
                className={cn(
                  ...
                  m.kind === "abono"
                    ...
                    : "bg-money/20 text-money-foreground",
                )}
              >
                {m.kind === "abono" ? (
              ...
              <span
                className={cn(
                  "cn-font-heading text-sm tabular-nums",
                  m.kind === "abono" ? "text-success" : "text-money-foreground",
                )}
              >
                {m.kind === "abono" ? "+" : "−"}
                {formatPts(m.points)}
      ...
      <section className="flex flex-col gap-3 rounded-3xl bg-money/10 p-4 ring-1 ring-money/20">
        ...
        {[
          {
            icon: Target,
            text: "Cada objetivo tiene una meta en Bs sobre una marca, familia o categoría: llegás a la meta y ganás sus puntos.",
          },
          {
            icon: ShoppingBag,
            text: "Tus compras se van sumando hasta la meta: no hace falta cumplirla en un solo pedido.",
          },
          {
            icon: Medal,
            text: "Subís de nivel con los puntos acumulados y cada objetivo te rinde más.",
          },
          {
            icon: Truck,
            text: "Canjeás productos desde el catálogo de canje: viajan en tu próximo pedido, sin costo en Bs.",
          },
        ].map(({ icon: Icon, text }) => (
